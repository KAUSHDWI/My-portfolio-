  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-6 z-10 flex flex-col md:flex-row items-center gap-12">
        ...
        >
          ...
          <motion.h1 
            ...
            className="text-4xl md:text-6xl font-bold text-white mb-6 tracking-tight leading-tight perspective-1000 relative"
          >
            {nameArray.map((char, index) => (
              <motion.span
                key={index}
                variants={letterVariants}
                className={`inline-block ${char === ' ' ? 'mr-4' : ''} hover:text-transparent hover:bg-clip-text hover:bg-gradient-to-r hover:from-cyan-400 hover:to-indigo-400 transition-all duration-300 cursor-default hover:scale-110`}
              >
                {char}
              </motion.span>
            ))}
          ...
          <motion.p variants={itemVariants} className="text-slate-300 max-w-lg mx-auto md:mx-0 mb-8 leading-relaxed text-lg bg-white/5 backdrop-blur-sm p-6 rounded-2xl border border-white/10 shadow-[0_0_30px_rgba(0,0,0,0.1)] hover:border-white/20 transition-colors">
            Building functional and modern applications for Android and the web.
            Expert in <span className="text-cyan-400 font-bold glow-text">Kotlin</span> & <span className="text-indigo-400 font-bold glow-text">React Native</span>.
          </motion.p>
          ...
          <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
            ...
            <a 
              ...
              target="_blank"
              rel="noreferrer"
              className="px-8 py-4 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/30 text-white rounded-xl flex items-center justify-center gap-2 transition-all group backdrop-blur-md shadow-lg hover:shadow-cyan-500/20"
            >
              <span>LinkedIn</span>
          ...
          <motion.div variants={itemVariants} className="mt-12 flex items-center justify-center md:justify-start gap-6">
            {SOCIAL_LINKS.map((link, idx) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="p-3 bg-white/5 rounded-full border border-white/10 hover:bg-gradient-to-br hover:from-cyan-500 hover:to-indigo-500 hover:border-transparent text-slate-400 hover:text-white transition-all transform hover:scale-125 hover:-translate-y-2 backdrop-blur-md shadow-lg hover:shadow-cyan-500/50"
              >
                <link.icon size={24} />
              </a>
            ))}
        ...
        {/* Visual Element - Right Side (Photo) */}
        <motion.div 
          className="flex-1 mt-8 md:mt-0 flex justify-center relative order-1 md:order-2"
           ...
           <div className="relative z-10 group perspective-1000">
             ...
             <div className="relative w-72 h-80 md:w-80 md:h-[28rem] bg-[#0f172a] rounded-[2.8rem] p-1.5 shadow-2xl rotate-3 group-hover:rotate-0 transition-all duration-500 ease-out overflow-hidden transform-style-3d hover:scale-[1.02] border-2 border-white/10">
                <div className="w-full h-full rounded-[2.5rem] overflow-hidden bg-slate-900 relative">
                  ...
                  {/* --- CLEANED UP IMAGE TAG HERE --- */}
                  <img 
                    src="/me.jpg"
                    alt={PERSONAL_INFO.name}
                    className="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-700"
                    onError={(e) => {
                      (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${encodeURIComponent(PERSONAL_INFO.name)}&background=0f172a&color=06b6d4&size=512`;
                    }}
                  />
             ...
             <motion.div 
                ...
                className="absolute -bottom-6 -right-4 md:-right-8 bg-white/10 backdrop-blur-2xl p-4 rounded-2xl border border-white/20 shadow-[0_8px_32px_0_rgba(31,38,135,0.37)] flex items-center gap-3 z-20 hover:scale-105 transition-transform cursor-default"
             >
      ...
      <motion.div 
        ...
        className="absolute bottom-10 left-1/2 -translate-x-1/2 text-slate-400 hidden md:block cursor-pointer hover:text-cyan-400 transition-colors p-3 bg-white/5 rounded-full backdrop-blur-sm border border-white/5 hover:bg-white/10"
